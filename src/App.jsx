...
const CODE_LANGUAGES = [
  ['', 'Plain text'], ['python','Python'], ['javascript','JavaScript'], ['typescript','TypeScript'],
  ['html','HTML'], ['css','CSS'], ['json','JSON'], ['bash','Bash / Shell'], ['sql','SQL'],
  ['java','Java'], ['c','C'], ['cpp','C++'], ['csharp','C#'], ['go','Go'], ['rust','Rust'],
  ['php','PHP'], ['kotlin','Kotlin'], ['swift','Swift'], ['xml','XML'], ['yaml','YAML'], ['markdown','Markdown'],
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (pendingContinuationFocus.current) {
      ...
      requestAnimationFrame(() => focusNode(continuationRefs.current.get(id)));
    }
  }, [document]);
  ...
  const changeType = (type, size = null) => {
    if (type === 'divider') {
      const index = document.blocks.findIndex(b => b.id === activeId);
      if (index < 0) return;
      ...
      setLanguageOpenId(null); setFormatOpen(false);
      requestAnimationFrame(() => focusBlock(activeId));
    }
  ...
  const setCodeLanguage = (id, language) => {
    ...
    setLanguageOpenId(null); setActiveId(id);
    tg()?.HapticFeedback?.selectionChanged?.();
    requestAnimationFrame(() => focusBlock(id));
  };
  ...
  const insertAfter = id => {
    ...
    const blocks = [...document.blocks];
    blocks.splice(blocks.findIndex(b => b.id === id) + 1, 0, nextBlock);
    commit({ ...document, blocks });
  ...
  const createParagraphAfterDivider = (dividerId, text) => {
    if (!text) return;
    const index = document.blocks.findIndex(b => b.id === dividerId);
    if (index < 0) return;
    const blocks = [...document.blocks];
    const following = blocks[index + 1];
    let nextBlock;
    if (following?.type === 'paragraph' && following.text === '') {
      nextBlock = { ...following, text };
      blocks[index + 1] = nextBlock;
    } else {
      nextBlock = createBlock('paragraph', 1, text);
  ...
  const removeBlock = id => {
    ...
    commit({ ...document, blocks: document.blocks.filter(b => b.id !== id) });
  ...
  const undo = () => {
    ...
    setActiveId(previous.blocks.find(b => b.id === activeId)?.id || previous.blocks[0].id);
    setFormatOpen(false); setLanguageOpenId(null); setHistoryVersion(v => v + 1);
  };
  ...
  const redo = () => {
    ...
    setDocument(next);
    setActiveId(next.blocks.find(b => b.id === activeId)?.id || next.blocks[0].id);
    setFormatOpen(false); setLanguageOpenId(null); setHistoryVersion(v => v + 1);
  };
  ...
  const send = async () => {
    ...
    if (!payload.blocks.length) { setError('Write something before sending.'); return; }
    setSending(true);
    try {
      const response = await fetch('/api/telegram', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ initData: app.initData, document: { version: 1, blocks: payload.blocks } }),
      });
  ...
  return <main className="app-shell">
    ...
    <section className="editor" onClick={() => { setFormatOpen(false); setLanguageOpenId(null); }}>
      <div className="document-area" onClick={event => {
        if (event.target === event.currentTarget) requestAnimationFrame(() => focusBlock(activeId));
      }}>
        {document.blocks.map((block, index) => <React.Fragment key={block.id}>
          <div className={`editor-block ${blockClass(block)} ${activeId === block.id ? 'active' : ''}`}
            onClick={event => { event.stopPropagation(); setActiveId(block.id); requestAnimationFrame(() => focusBlock(block.id)); }}>
            {block.type === 'divider' ? <div className="divider-line" role="separator" aria-label="Divider"/> : <>
              {block.type === 'pre' && <div className="code-tools" onClick={event => event.stopPropagation()}>
                <button type="button" className="code-language-button" aria-label="Set code language" aria-expanded={languageOpenId === block.id}
                  onClick={() => { setActiveId(block.id); setLanguageOpenId(open => open === block.id ? null : block.id); setFormatOpen(false); }}>
                ...
                {languageOpenId === block.id && <div className="code-language-menu" onClick={event => event.stopPropagation()}>
                  {CODE_LANGUAGES.map(language => <button type="button" key={language.value || 'plain'} className={`code-language-option ${block.language === language.value ? 'active' : ''}`} onClick={() => setCodeLanguage(block.id, language.value)}>
                    <span>{language.label}</span>{block.language === language.value && <Icon name="check" size={18}/>} 
                  </button>)}
                </div>}
              </div>}

              {block.type === 'pre' ? <textarea
                ref={node => { if (node) editorRefs.current.set(block.id, node); else editorRefs.current.delete(block.id); }}
                className="editable code-editor" value={block.text} rows={4} wrap="off" spellCheck={false}
                aria-label="Code block" aria-multiline="true" placeholder={placeholder(block)}
                onFocus={() => setActiveId(block.id)} onClick={event => event.stopPropagation()}
                onChange={event => updateText(block.id, event.currentTarget.value)} onKeyDown={event => handleKeyDown(event, block)}
              /> : <div
                ref={node => { if (node) editorRefs.current.set(block.id, node); else editorRefs.current.delete(block.id); }}
                className="editable" contentEditable suppressContentEditableWarning spellCheck role="textbox" aria-multiline="true"
                aria-label={block.type === 'heading' ? `Heading ${block.size}` : block.type === 'footer' ? 'Footer' : 'Paragraph'}
                data-placeholder={placeholder(block)} onFocus={() => setActiveId(block.id)} onClick={event => event.stopPropagation()}
                onInput={event => updateText(block.id, event.currentTarget.textContent || '')} onKeyDown={event => handleKeyDown(event, block)}
              />}
            </>}
          </div>

          {block.type === 'divider' && index === document.blocks.length - 1 && <div className="divider-continuation" onClick={event => event.stopPropagation()}>
            <div ref={node => { if (node) continuationRefs.current.set(block.id, node); else continuationRefs.current.delete(block.id); }}
              className="continuation-editable" contentEditable="plaintext-only" suppressContentEditableWarning role="textbox" aria-multiline="true"
              aria-label="Text below divider" data-placeholder="Write here…" onFocus={() => setActiveId(block.id)}
              onInput={event => { const text = event.currentTarget.innerText.replace(/\r\n/g, '\n').trimEnd(); if (text) createParagraphAfterDivider(block.id, text); }}
            />
          </div>}
        </React.Fragment>)}
      </div>
    ...
    <footer className="composer-bar"><div className="toolbar"><div className="format-wrap">
      <button className={`tool-button heading-tool ${formatOpen ? 'selected' : ''}`} aria-label="Text and block formatting" aria-expanded={formatOpen}
        onClick={event => { event.stopPropagation(); setLanguageOpenId(null); setFormatOpen(open => !open); }}>H</button>
