...
function contentSelection(node){
  ...
  const range=selection.getRangeAt(0),walker=document.createTreeWalker(node,NodeFilter.SHOW_TEXT);
  let start=0,end=0,n,foundStart=false,foundEnd=false;
  while((n=walker.nextNode())){
    if(n===range.startContainer){start+=range.startOffset;foundStart=true}
    else if(!foundStart)start+=n.textContent?.length||0;
    if(n===range.endContainer){end+=range.endOffset;foundEnd=true;break}
    else if(!foundEnd)end+=n.textContent?.length||0;
  }
...
const common={id:b.id,text:b.text||'',placeholder:b.structural?'Write something…':b.type==='heading'?'Heading '+b.size:b.type==='footer'?'Footer':b.type==='blockquote'||b.type==='expandable_blockquote'||b.type==='pullquote'?'Quote':'Write something…',className:b.type==='heading'?'heading h-'+b.size:b.type==='footer'?'footer':''};
