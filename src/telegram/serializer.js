import{runsToRichText}from'../document/runs.js';
...
function blockRichText(b){if(Array.isArray(b?.runs)&&b.runs.length){const richText=runsToRichText(b.runs);return Array.isArray(richText)?richText:[richText]}return richFromMarks(b.text,b.inlineMarks);}
export function serializeBlock(b){switch(b.type){
case'paragraph':case'footer':return{type:b.type,text:blockRichText(b)};
case'heading':return{type:'heading',text:blockRichText(b),size:b.size};
case'pre':return{type:'pre',text:blockRichText(b),...(b.language?{language:b.language}:{})};
...
case'expandable_blockquote':return{type:'expandable_blockquote',text:blockRichText(b),...(b.credit?{credit:rich(b.credit)}:{})};
case'pullquote':return{type:'pullquote',text:blockRichText(b),...(b.credit?{credit:rich(b.credit)}:{})};
